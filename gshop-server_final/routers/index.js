...
router.get('/index_category', function(req, res) {
  setTimeout(function () {
    const data = require('../data/kingkong.json')
    console.log(data)
    res.send({code: 0, data})
  }, 300)
})
...
/* 购物车商品列表 */
router.get("/cartShops",function(req,res){
    setTimeout(function () {
        const data = require('../data/cartShops.json')
        res.send({code: 0, data})
      }, 300)
})
//店铺详情
router.get("/food",function(req,res){
    const {id} = req.query
    console.log(id)
    if(id==="1007192535729592"){
        setTimeout(function () {
            const data = require('../data/food.json')
            res.send({code: 0, data,msg:"1007192535729592"})
          }, 300)
    }else{
        return res.send({code: 1, msg: 'id错误'})
    }
})
//热搜标签
router.get("/hot",function(req,res){
        setTimeout(function () {
            const data = require('../data/hotList.json')
            res.send({code: 0, data})
          }, 300)
...
router.get("/rating",function(req,res){
        ...
        if(id==="1007192535729592"){
            setTimeout(function () {
                const data = require('../data/rating.json')
                res.send({code: 0, data})
              }, 300)
        }
